#!/usr/bin/env node
/**
 * BlackDelta — Solana DLMM LP Agent CLI
...
// ─── Load .env from ~/.blackdelta/ (fallback to ~/.meridian/) ───
const blackdeltaDir = path.join(os.homedir(), ".blackdelta");
const meridianDir = path.join(os.homedir(), ".meridian");
let targetDir = fs.existsSync(blackdeltaDir) ? blackdeltaDir : (fs.existsSync(meridianDir) ? meridianDir : blackdeltaDir);

const configEnv = path.join(targetDir, ".env");
...
// ─── SKILL.md generation ──────────────────────────────────────────
const SKILL_MD = `# BlackDelta — Solana DLMM LP Agent CLI

Data dir: ~/.blackdelta/ (or ~/.meridian/)
...
### blackdelta screen [--dry-run] [--silent]
Runs one AI screening cycle to find and deploy new positions.
\`\`\`
Output: { done: true, report: "..." }
\`\`\`

### blackdelta manage [--dry-run] [--silent]
Runs one AI management cycle over open positions.
\`\`\`
Output: { done: true, report: "..." }
\`\`\`

### blackdelta deploy --pool <addr> --amount <sol> [--bins-below 69] [--bins-above 0] [--strategy bid_ask|spot] [--dry-run]
...
### blackdelta search-pools --query <name_or_symbol> [--limit 10]
...
### blackdelta blacklist add --mint <addr> --reason <text>
...
switch (subcommand) {
  ...
  case "pnl": {
    ...
    if (!positionAddress) die("Usage: blackdelta pnl <position_address>");
  ...
  case "token-info": {
    ...
    if (!query) die("Usage: blackdelta token-info --query <mint_or_symbol>");
  ...
  case "token-holders": {
    ...
    if (!mint) die("Usage: blackdelta token-holders --mint <addr>");
  ...
  case "token-narrative": {
    ...
    if (!mint) die("Usage: blackdelta token-narrative --mint <addr>");
  ...
  // ── pool-detail ───────────────────────────────────────────────
  case "pool-detail": {
    if (!flags.pool) die("Usage: blackdelta pool-detail --pool <addr> [--timeframe 5m]");
  ...
  case "search-pools": {
    ...
    if (!query) die("Usage: blackdelta search-pools --query <name_or_symbol>");
  ...
  // ── active-bin ────────────────────────────────────────────────
  case "active-bin": {
    if (!flags.pool) die("Usage: blackdelta active-bin --pool <addr>");
    const { getActiveBin } = await import("./tools/dlmm.js");
  ...
  case "wallet-positions": {
    ...
    if (!wallet) die("Usage: blackdelta wallet-positions --wallet <addr>");
  ...
  // ── deploy ───────────────────────────────────────────────────────
  case "deploy": {
    if (!flags.pool) die("Usage: blackdelta deploy --pool <addr> --amount <sol>");
  ...
  // ── claim ────────────────────────────────────────────────────────
  case "claim": {
    if (!flags.position) die("Usage: blackdelta claim --position <addr>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  // ── close ────────────────────────────────────────────────────────
  case "close": {
    if (!flags.position) die("Usage: blackdelta close --position <addr>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  // ── swap ─────────────────────────────────────────────────────────
  case "swap": {
    if (!flags.from || !flags.to || !flags.amount) die("Usage: blackdelta swap --from <mint> --to <mint> --amount <n>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  case "config": {
    if (sub2 === "get" || !sub2) {
      ...
      if (!key || rawVal === undefined) die("Usage: blackdelta config set <key> <value>");
      let value = rawVal;
  ...
  // ── study ────────────────────────────────────────────────────────
  case "study": {
    if (!flags.pool) die("Usage: blackdelta study --pool <addr> [--limit 4]");
  ...
  case "start": {
    ...
    process.stderr.write("[blackdelta] Starting autonomous agent...\n");
    startCronJobs();
    break;
  }
  ...
  case "lessons": {
    if (sub2 === "add") {
      ...
      if (!text) die("Usage: blackdelta lessons add <text>");
  ...
  // ── pool-memory ──────────────────────────────────────────────────
  case "pool-memory": {
    if (!flags.pool) die("Usage: blackdelta pool-memory --pool <addr>");
  ...
  case "blacklist": {
    if (sub2 === "add") {
      if (!flags.mint) die("Usage: blackdelta blacklist add --mint <addr> --reason <text>");
  ...
  case "withdraw-liquidity": {
    if (!flags.position) die("Usage: blackdelta withdraw-liquidity --position <addr> --pool <addr> [--bps 10000]");
  ...
  // ── add-liquidity ──────────────────────────────────────────────
  case "add-liquidity": {
    if (!flags.position) die("Usage: blackdelta add-liquidity --position <addr> --pool <addr> [--amount-x <n>] [--amount-y <n>]");
    if (!flags.pool) die("--pool is required");
  ...
  default:
    die(`Unknown command: ${subcommand}. Run 'blackdelta help' for usage.`);
}
